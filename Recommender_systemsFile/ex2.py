...
def best_movies(inv):
    results = []
    for movie, rating in inv.items():
        pom = list(rating.values())
        average = sum(pom)/len(pom)
        results.append((average, movie))
...
if __name__ == '__main__':
    ...
    recommendation_pears = get_recommendations(ratings, user_name)
    recommendation_distance = get_recommendations(ratings, user_name, similarity=sim_distance)

    if recommendation_pears[0][1] == recommendation_distance[0][1]:
        print(recommendation_pears[0][1])
    else:
        transformed_ratings = transform_prefs(ratings)
        recommendation_transformed = get_recommendations_item_based(transformed_ratings, user_name)
        if recommendation_transformed[0][1] == recommendation_pears[0][1] or recommendation_transformed[0][1] == \
                recommendation_distance[0][1]:
            print(recommendation_transformed[0][1])
        else:
            print(best_movies(transformed_ratings))
