...
if __name__ == '__main__':
    ...
    pears_documents = rank_documents(text, documents, sim_func=pearson)
    cosine_documents = rank_documents(text, documents)

    same = True
    for i in range(0,4):
        print(f'Rang po slicnost:{i} Cosine-Document #{cosine_documents[i][1]} Pearson-Document #{pears_documents[i][1]}')
        if cosine_documents[i][1] != pears_documents[i][1]:
            same = False

    print(f'Dvete metriki davaat ist rezultat:{same}')