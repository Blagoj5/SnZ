...
if __name__ == '__main__':
    ...
    tf_idf_data = create_dataset(documents, labels)

    tree = build_tree(tf_idf_data[0])
    tree_sample = process_document(sample, tf_idf_data[1], tf_idf_data[2], tf_idf_data[3])

    tree_prediction = max_prediction(classify(tree_sample, tree))
    ...
    naive_prediction = c1.classify_document(sample)

    if tree_prediction == naive_prediction:
