...
class DocumentClassifier:
    def __init__(self, get_features):
        # број на парови атрибут/категорија (fe    sample = input()ature/category)
...
if __name__ == '__main__':
    ...
    tf_idf_dataset = create_dataset(documents, labels)
    tree = build_tree(tf_idf_dataset[0])

    tp = 0.0 # tochni spam poraki
    fp = 0.0 # greshni spam poraki
    tn = 0.0 # tocni ham poraki
    fn = 0.0 # greshni ham poraki
    for row in test_data:

        tf_idf_sample = process_document(row[0], tf_idf_dataset[1], tf_idf_dataset[2], tf_idf_dataset[3])
        prediction = max_prediction(classify(tf_idf_sample, tree))
    ...
    print(f'Tochnost: {(tp + tn)/(tp + fp + tn + fn)}')
    print(f'Preciznost: {(tp)/(tp + fp)}')
    print(f'Odziv: {(tp)/(tp + fn)}')
    tf_idf_sample = process_document(test_comment, tf_idf_dataset[1], tf_idf_dataset[2], tf_idf_dataset[3])
    print(max_prediction(classify(tf_idf_sample, tree)))