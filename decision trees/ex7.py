...
if __name__ == '__main__':
    ...
    tree1 = build_tree(dataset[:int(len(dataset)*0.3)])
    tree2 = build_tree(dataset[int(len(dataset)*0.3):int(len(dataset)*0.6)])
    tree3 = build_tree(dataset[int(len(dataset)*0.6):])

    vote = {0: 0, 1: 0, 2: 0}

    classification = get_max_prediction(classify(test_case, tree1))
    classification2 = get_max_prediction(classify(test_case, tree2))
    classification3 = get_max_prediction(classify(test_case, tree3))

    vote[classification] += 1
    vote[classification2] += 1
    vote[classification3] += 1

    print('Glasovi: ' + str(vote))

    sortedvode = sorted(vote.items(), key=lambda item: item[1], reverse=True)

    if sortedvode[0][1] == sortedvode[1][1]:
        print("unknown")
    else:
        print('Predvidena klasa: ' + str(sortedvode[0][0]))
