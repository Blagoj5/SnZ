# Изградете 2 дрва на одлука. Едното дрво на одлука ќе ја користи првата половина од 
# податочното множество, а другото дрво, втората половина. Доколку двете дрва на одлука 
# на тест примерот го дадат истиот резултат, да се испечати тој резултат. Доколку дадат
# различен резултат, да се испечати KONTRADIKCIJA. Доколку некое од дрвата има само една 
# класа тогаш се предвидува истата, но ако има повеќе од една треба да се избере таа со
# најголем број на инстанци. Ако во листот има неколку класи со ист број на инстанци да
# се предвиде првата класа по азбучен ред.
...
def get_max_prediction(classification_dictionary):
...
if __name__ == "__main__":
    ...
    length = len(trainingData)/2

    mnoz = trainingData[:int(length)]
    monz2 = trainingData[int(length):]
    tree = build_tree(mnoz)
    tree2 = build_tree(monz2)

    print(tree.results)

    saver = classify(testCase, tree)
    saver2 = classify(testCase, tree2)

    if saver.keys() == saver2.keys():
        print([key for key in saver.keys()][0])
    else:
        print('KONTRADIKCIJA')
