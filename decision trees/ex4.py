...
#Da vrati maximumot odnosno go gleda vtoriot element od kategorijata, toes kolku ima takvi ribi i go sporedva so drug ako ima poveke vo toj dict
# so lamda definirame da sporedva spored vtoriot element odnosno spored brojot 
def maximum(classification):
...
def max_prediction(classification):
    return max(classification.items(), key= lambda item: item[1])[0]
...
if __name__ == "__main__":
    ...
    # test_data = [train_data[i] for i in range(len(train_data)) if i in test_indices]  #Testirackoto mnozestvo
    # train_data = [train_data[i] for i in range(len(train_data)) if i not in test_indices]  #Moze i vaka za brishenje na tie elementi mnozestvo
    ...
    for test in test_indices:
        # train_data.remove(int(test))  //****REMOVES THE SPECIFIC VALUE*****
        if warrup:
            warrup = False
        else:
            count += 1
            test = int(test) - count
        try:
            test_data.append(train_data[test])
            del train_data[int(test)]           # //***REMOVES THE SPECIFIC INDEX***** 
        except:
            continue

    lenght = len(train_data) // 2

    tree = build_tree(train_data[:lenght])
    tree2 = build_tree(train_data[lenght:])


    first_tree_correct = 0.0
    second_tree_correct = 0.0
    for row in test_data:

        tree_prediction = max_prediction(classify(row, tree))
        tree2_prediction = max_prediction(classify(row, tree2))

        if tree_prediction == row[-1]:
            first_tree_correct += 1
        if tree2_prediction == row[-1]:
            second_tree_correct += 1

    tocnost_tree = first_tree_correct/len(test_data)
    tocnost_tree2 = second_tree_correct/len(test_data)

    print('Tochnost na drvoto 1: ' + str(tocnost_tree))
    print('Tochnost na drvoto 2: ' + str(tocnost_tree2))

    if tocnost_tree == tocnost_tree2:
        print('Drvata imaat ista tochnost')
    elif tocnost_tree > tocnost_tree2:
...
#Изградете 2 дрва на одлука. Едното дрво на одлука ќе ја користи првата 
# половина од податочното множество, а другото дрво, втората половина. 
# Притоа, од првичното податочно множество за тренирање отстранете ги 
# редиците чии што индекси ги содржи листата test_indices и од тие 
# податочни редици креирајте го тестирачкото множество. Потоа, пресметајте
#  ја точноста на двете дрва на одлука со користење на тестирачкото множество.
#  Доколку двете дрва имаат иста точност да се испринта 
# "Drvata imaat ista tochnost", а во спротивно потребно да е да се испринта
#  "Drvoto i e podobro od drvoto j".

# Точност на класификатор се пресметува како:

# точност = број на точни примероци / вкупен број на примероци