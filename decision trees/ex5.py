...
class DecisionNode:
    def __init__(self, col=-1, value=None, results=None, tb=None, fb=None, level=None):
        """
...
def build_tree(rows, level, scoref=entropy):
    ...
    if best_gain > 0:
        ...
        return DecisionNode(col=best_criteria[0], value=best_criteria[1],
                            tb=true_branch, fb=false_branch, level=level)
    else:
...
if __name__ == "__main__":
    ...
    half = len(training_data) // 2
    tree = build_tree(training_data[:half], 0)

    tree2 = build_tree(training_data[half:], 0)

    print_tree(tree)
    print_tree(tree2)

    if maximum(classify(testCase, tree)) == maximum(classify(testCase, tree2)):
        print(maximum(classify(testCase, tree)))
    else:
        print('KONTRADIKCIJA')

# Да се промени алгоритмот за дрво на одлука така што ќе се изградат
# две дрва на одлука. Секое од дрвата го користи половина од податочното множество.

# Да се промени начинот на печатење на дрвото така што покрај секој јазол, 
# ќе се испечати и неговото ниво.

# Двете дрва да се испечатат и потоа да се испечати резултатот од класификацијата.

# Доколку некое од дрвата има само една класа тогаш се предвидува истата, но ако има 
# повеќе од една треба да се избере таа со најголем број на инстанци. Ако во листот има
