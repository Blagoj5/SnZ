# Да се направи модел за класификација за даденото податочно множество. За тренинг да се земат само првите 25 примероци од класите #Bream и Perch (Првите 25 од Bream и првите 25 од Perch или вкупно 50). Значи треба да се направи бинарен класификатор, при што секоја #класа освен Bream и Perch се игнорира при тренирањето на множеството. Притоа земањето на првите 25 примероци да се направи во #програмата, а не со рачно копирање!
...
def classify(observation, tree):
    ...
    if tree.results:
        ...
        if compare(observation, tree.col, tree.value):
            branch = tree.tb
            print("True branch:" + str(branch))
        else:
            branch = tree.fb
            print("False branch:" + str(branch))
...
if __name__ == "__main__":
    ...
    set1 = [row for row in data if
            row[-1] == "Bream"]
    set1 = set1[0:26]

    set2 = [row for row in data if
            row[-1] == "Perch"]
    set2 = set2[0:26]
    tree = build_tree(set1 + set2)

    classifytest = classify(test_case, tree)
    print("Class")
    print(classifytest)

    predicted_class = get_max_prediction(classify(test_case, tree))
