...
def get_words_second(doc, emotic):
    """Поделба на документот на зборови. Стрингот се дели на зборови според
    празните места и интерпукциските знаци
...
if __name__ == '__main__':
    ...
    test_data = data[sample_ind]
    print(test_data[1])
    print('Vistinska klasa: ' + test_data[0])
    del data[sample_ind]

    # ILI
    # test_data = data.pop(sample_ind) // VAKA I GO IZVADOVME SO POP ELEMENTOT KOJ NE TREBA
    # print(test_data)

    c2 = NaiveBayes(lambda doc: get_words_second(doc, emoticons))
    c1 = NaiveBayes(get_words)

    for row in data:
        c1.train(row[1], row[0])
        c2.train(row[1], row[0])

    print('Klasa predvidena so Naiven Baes (bez emotikoni): ' + str(c1.classify_document(test_data[1])))
    print('Klasa predvidena so Naiven Baes (so emotikoni): ' + str(c2.classify_document(test_data[1])))

# Потребно е да изградите два класификатори со Наивен Баес за класификација на документи кои се постови од
# социјалната мрежа Twitter. Класите на документите се позитивен или негативен сентимент. Податоците се дадени во
# променливата data, како торки каде што првиот елемент е класата, а вториот елемент е документот. Од влез во
# променливата sample_ind се сместува индексот на документот кој треба да се предвиди со креираните модели. Овој
# документ се вади од множеството и служи за тестирање, а остатокот е множество за тренирање.

# Првиот класификатор треба да ја користи стандарната функција за делба на зборови get_words која е дадена на
# аудиториските вежби. Вториот класификатор треба да користи втора верзија на оваа функција која дополнително ќе
# вклучува емотикони од текстот. Оваа нова функција треба да ја користи истата логика за извлекување на зборовите
# како и функцијата get_words, каде што потоа дополнително се прави проверка на емотиконите. За да ги добиете
# емотиконите потребно е дадениот документ да се подели по празни места. За да го поделите документот според празни
# места користете ја функцијата split дефинирана на документи во кој аргументот кој ќе го предадете е стринг составен
# од празно место (пример: document.split(' ')). Потоа, во резултатот кој се враќа од функцијата потребно е да се
# вклучат зборовите од кои е составен документот и емотиконите кои ги содржи (се вклучуваат само емотиконот/ите кои
# се предефинирани во променливата emoticons доколку се појавуваат во документот).
