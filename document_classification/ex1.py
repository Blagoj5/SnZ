...
def get_words_ignore(doc, words_ignore):
    words = set()
    for word in re.split('\\W+', doc):
...
def funkcija(doc):
    return get_words_ignore(doc, words_to_ignore)
...
if __name__ == "__main__":
    ...
    c2 = NaiveBayes(funkcija)

    for row in train_data:
        c1.train(row[0], row[1])
        c2.train(row[0], row[1])

    klasa1 = c1.classify_document(recenica)
    klasa2 = c2.classify_document(recenica)
...
# документ get-words треба да се промени така што ќе прима втор опционален аргумент words_to_ignore. Ако не се проследи
...
# истренира и втор класификатор кој ќе го употребува истото тренинг множество, но притоа ќе ја употребува новата
# функција која е веќе имплементирана get_words_with_ignore.
