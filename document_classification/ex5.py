...
def get_words_upgrade(doc, words_include):
    """Поделба на документот на зборови. Стрингот се дели на зборови според
    празните места и интерпукциските знаци
    ...
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word.lower() in words_include:
            words.add(word.lower())
    return words
...
if __name__ == '__main__':
    ...
    c2 = NaiveBayes(lambda doc: get_words_upgrade(doc, words_to_include))

    for row in data:
        c1.train(row[0], str(row[1]))
        c2.train(row[0], str(row[1]))

    print('Klasa predvidena so site zborovi: ' + c1.classify_document(comment))
    print('Klasa predvidena so samo kluchni zborovi: ' + c2.classify_document(comment))

# Во променливата data дадено е податочно множество од коментари за филмови кои се означени со сентимент. Документот
# може да има негативен (0) или позитивен (1) сентимент. Направете два класификатори на Наивен баес. Првиот
# класификатор треба да ја користи функцијата get_words (од дадената рамка) за поделба на документот на зборови.
# Вториот класификатор треба да ги дели зборовите на истиот начин, но како резултат од функцијата се враќаат само
# зборовите кои се наоѓаат во листата на зборови words_to_include.

# Од стандарден влез се чита еден коментар за кој треба да се одреди сентиментот (променлива comment). Предодредена
# класа при класификацијата е 'unknown'. На излез испринтајте ги предвидените класи за сентимент со двата
# класификатори.