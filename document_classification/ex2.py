...
if __name__ == '__main__':
    ...
    data1 = train_data[:int(len(train_data) * 0.4)]  # DATA FOR LEARNING
    data2 = train_data[int(len(train_data) * 0.4):int(len(train_data) * 0.8)]  # DATA FOR LEARNING
    data3 = train_data[int(len(train_data) * 0.8):]  # TRAINING DATA

    for row in data1:
        c1.train(row[0], row[1])

    for row in data2:
        c2.train(row[0], row[1])

    tocnost = 0.0
    for row in data3:
        klasa1 = c1.classify_document(row[0])
        klasa2 = c2.classify_document(row[0])

        if klasa1 == klasa2 and row[-1] == klasa1:
            tocnost += 1

    print(tocnost / len(data3))

    if c1.classify_document(sample) == c2.classify_document(sample):
        print(c1.classify_document(sample))
    else:
        print('unknown')
...
# користи првите 40% од документите во податочното множество. Вториот класификатор ги користи вторите 40% од
# податочното множество. Останатите 20% од податочното множество се користат за тестирање. Тренирајте ги двата
# класификатори со соодветното подмножество.

# Точноста на класификаторот се пресметува на следниот начин: број на точно класифицирани документи од двата
# поединечни класификатори / вкупен број на документи во множеството за тестирање (одреден документ се смета за точно
# класифициран доколку со двата поединечни класификатори се добие вистинската класа)

# Испечатете ја точноста на наивниот баесов класификатор. Потоа, за документот прочитан од стандарден влез испечатете
# ја класата во која тој припаѓа. Доколку со двата поединечни класификатори се добие истата класа, тогаш се печати
# таа. Во спротивно се печати непозната (unknown) класа.
