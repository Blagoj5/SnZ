...
if __name__ == "__main__":
    ...
    # c.classify_document(recenica)

    words = sorted(get_words(recenica))

    for word in words:
        print(str(word) + ' science ' + str(round(c.get_feature_per_category_probability(word, 'science'), 5)) +
              ' ' + str(round(c.weighted_probability(word, 'science', c.get_feature_per_category_probability), 5)))
        print(str(word) + ' sport ' + str(round(c.get_feature_per_category_probability(word, 'sport'), 5)) +
              ' ' + str(round(c.weighted_probability(word, 'sport', c.get_feature_per_category_probability), 5)))

    klasa = c.classify_document(recenica)  # KE BIDI ILI SCIENCE ILI SPORT
    log = round(log2(c.get_category_probability_for_document(recenica, klasa)), 5)
    print(klasa, log)
...
# стринг, а втор елемент е класата како стринг. Да се истренира модел врз основа на тренинг множеството. Потоа,
# за секој прочитан документ од стандарден влез да се испечатат зборовите кои се употребуваат за класификација,
# класата и веројатноста на зборот да е од таа класа (заокружено на 5 децимали), како и тежинската веројатност на
# зборот да припаѓа на класата (заокружено на 5 децимали). На крај да се испечати предвидената класа на документот и
# логаритам со основа 2 од веројатноста со која се предвидува (заокружено на 5 децимали). Забелешка: зборовите
# подредете ги по азбучен редослед.
