...
def get_words(doc):
    ...
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and fivewords <= 5:
...
if __name__ == '__main__':
    ...
    testData = train_data[int(len(train_data)*0.7):]
    ...
    for row in testData:
        klasa = c.classify_document(row[0])

        if klasa == row[-1]:
            tochnost += 1

    print(tochnost/len(testData))
    print(c.classify_document(sample))





# Изградете наивен баесов класификатор кој ќе одредува дали даден документ е формален или неформален. Класификацијата
# се прави користејќи ги само првите 5 зборови од речениците. Класификаторот ги користи првите 70% од документите во
# податочното множество. Останатите 30% од податочното множество се користат за тестирање.

# Точноста на класификаторот се пресметува на следниот начин: број на точно класифицирани документи / вкупен број на
# документи

# Испечатете ја точноста на наивниот баесов класификатор. Потоа, за документот прочитан од стандарден влез испечатете
# ја класата во која тој припаѓа.