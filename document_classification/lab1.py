...
"""
     , "sport"),
    ("""When St. John's landed Rysheed Jordan, the consensus was that he would be an excellent starter. So far, 
    that's half true. Jordan came off the bench Sunday and tied a career high by scoring 24 points to lead No. 24 St. 
    John's to a 74-53 rout of Fordham in the ECAC Holiday Festival. ''I thought Rysheed played with poise,
    '' Red Storm coach Steve Lavin said. ''Played with the right pace. Near perfect game.'' """
     , "sport"),
    ("""Five-time world player of the year Marta scored three goals to lead Brazil to a 3-2 come-from-behind win over 
    the U.S. women's soccer team in the International Tournament of Brasilia on Sunday. Carli Lloyd and Megan Rapinoe 
    scored a goal each in the first 10 minutes to give the U.S. an early lead, but Marta netted in the 19th, 
    55th and 66th minutes to guarantee the hosts a spot in the final of the four-team competition. """
     , "sport"),
]
...
if __name__ == "__main__":
    ...
    cClass = c.classify_document(recenica)
    print(cClass)

    if cClass == 'science':
        # probability = c.get_category_probability_for_document(recenica, 'science')
        probability = c.get_category_probability_for_document(recenica, cClass)
    else:
        # probability = c.get_category_probability_for_document(recenica, 'sport')
        probability = c.get_category_probability_for_document(recenica, cClass)
...
# стринг, а втор елемент е класата како стринг. Да се истренира модел врз основа на тренинг множеството и потоа за
# секој документ прочитан од стандарден влез да се испечати неговата класа. Потоа да се испечати веројатноста за
# одредената класа на документот до 10 децимали. Заокружувањето да се прави со користење на функцијата round().
# Пример: round(probability, number_of_decimal_places)
